...
pub(crate) fn format_report_html(
    ...
    h.push_str(
        ".tree-toolbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 12px}\n",
    );
    ...
    h.push_str(
        ".tree-scroll{max-width:100%;overflow-x:auto;overflow-y:hidden;padding-bottom:8px}\n",
    );
    h.push_str(
        ".tree-browser{display:inline-block;min-width:100%;width:max-content;padding-right:16px}\n",
    );
    h.push_str(
        ".tree-node{margin:0;border-left:3px solid transparent;transition:border-color .15s}\n",
    );
    ...
    h.push_str(
        ".tree-node.tree-match>summary,.tree-text.tree-match{background:#fef3c7;color:#92400e}\n",
    );
