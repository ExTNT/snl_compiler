...
- 递归下降恢复出的 AST 可继续用于语义分析和 HTML 报告，但任何语法错误都会阻止代码生成
...
procedure 栈帧:
  $fp → [保存的旧 $fp]     0($fp)
         [保存的 $ra]       4($fp)
         [静态链]            8($fp)
         参数 (调用者压入)  12($fp) = 第一个参数
         [后续参数...]       16($fp), ...（按参数槽大小递增）
         [局部变量...]       -4($fp), -8($fp), ...
```

调用者从右到左压入显式实参，再压入隐藏静态链。标量值参数占 4 字节，`var` 参数传地址，数组和记录值参数按完整对象大小复制。

### 6.4 变量访问策略

| 变量层级 | 存储位置 | 访问方式 |
|---------|---------|---------|
| 全局变量 (level 0) | `.data` 段 | `la $t0, var_X` → `lw/sw/lb/sb $v0, 0($t0)` |
| 当前过程局部变量 | 栈上 `$fp` 负偏移 | `lw/sw/lb/sb $v0, offset($fp)` |
| 外层过程局部变量 | 外层过程栈帧 | 沿 `8($fp)` 静态链到声明层级后按偏移访问 |
| 值参数 | 当前或外层过程栈帧 | 标量直接按正偏移访问；复合值以参数槽首地址访问 |
| `var` 参数 | 当前或外层过程栈帧 | 从参数槽加载调用者地址后间接访问 |
...
`$fp` 属于被调用者保存寄存器。过程序言保存旧的 `$fp`，尾声恢复；调用者还在每次调用中传入隐藏静态链。内层过程沿静态链定位外层词法作用域的活动记录，从而正确访问被捕获的局部变量。过程汇编标签包含完整词法路径，避免不同作用域的同名嵌套过程冲突。
