...
- **168 个单元测试**全部通过（词法分析 29 个、语法分析 47 个、AST 显示 1 个、语义分析 29 个、代码生成 45 个、HTML 报告 17 个）
- **25 个样例程序**全部批量编译通过，生成对应 MIPS 汇编和 HTML 诊断报告
...
3. **统一测试验证**：建立 25 个覆盖基础语法、算法（递归/循环/查找/排序/滑动窗口/贪心/动态规划）、复合数据类型（数组/记录）的 SNL 样例程序，作为全组共用的集成测试集。
...
- **边界处理**：孤立 `:`、非法字符、整数溢出以及未闭合的字符/注释均显式报告词法错误
...
- **栈帧约定**：`main` 帧保存 `$ra`；过程帧保存 `$fp` + `$ra`，并在 `8($fp)` 保存隐藏静态链
- **参数传递**：调用者将实参逆序压栈，再压入静态链；标量值参数占一个字，`var` 参数传地址，数组和记录值参数复制完整对象；返回值统一在 `$v0`
- **表达式求值**：右操作数先求值压栈 → 左操作数求值 → 弹出右操作数 → 运算
- **全局变量**：在 `.data` 段声明标签，通过 `la $t0, var_X` 访问
- **嵌套过程支持**：调用沿静态链访问外层变量，汇编标签按完整词法路径限定；递归调用通过标准 `jal`/`jr $ra` 实现
- **复合值与返回**：数组/记录赋值执行完整字节复制；`return` 设置 `$v0` 后跳转到共享过程尾声
...
| 查找 | `linearsearch.snl` | `5` `5` | 线性查找（目标值 + 索引） |
| 查找 | `binarysearch.snl` | `38` `6` | 二分查找（目标值 + 索引） |
| 排序 | `mergesort.snl` | `1` `3` `9` `10` `27` `38` `43` `82` | 递归归并排序 |
| 扫描 | `maxsubarray.snl` | `4` | 最长递增连续子数组长度 |
| 滑动窗口 | `slidingwindow.snl` | `17` | 大小为 k=3 的最大子数组和 |
| 贪心 | `greedycoin.snl` | `10` `3` `5` `1` `2` `1` `1` `0` `0` | 贪心硬币找零（面额 数量对） |
| 动态规划 | `dpcoin.snl` | `15` `3` | DP 最少硬币找零 |
| 动态规划 | `dpknapsack.snl` | `10` `15` | DP 0/1 背包 |
...
`report.rs` 负责生成 `*_report.html`：内嵌 CSS/JS，渲染 Token 表、语法树浏览器、符号表和各阶段错误信息。**语法树节点按语义类别用颜色标识**（钢蓝 = 声明、翠绿 = 语句、珊瑚 = 表达式），支持横向滚动、展开/折叠、搜索过滤和匹配高亮。报告采用暗色工业终端美学风格，全部使用 CSS 自定义属性，相关 HTML 转义与报告结构测试也集中在该模块。
